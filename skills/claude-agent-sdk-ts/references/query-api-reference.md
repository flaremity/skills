...
| `system` (subtype: `task_started`) | Task spawned (v0.2.50+) | `task_id`, `description`, `uuid`, `session_id` (v0.2.52+) |
| `system` (subtype: `task_progress`) | Task progress update (v0.2.52+) | `task_id`, `description`, `usage` (`total_tokens`, `tool_uses`, `duration_ms`), `tool_use_id` (optional), `last_tool_name` (optional) |
